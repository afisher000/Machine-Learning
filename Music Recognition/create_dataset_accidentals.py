...
orig = uio.import_song(f'Sheet Music\\No Staff\\{song_name}.jpg', save_params=False)
...
while keep_looping:
    ...
    if key == 2555904:
        ...
        j -= 1
    elif key in map(ord, list('sfnmbtdrz')):
        labels[j] = chr(key)
        print('Fill')
        j += 1        
    elif key == ord('q'):
...
file_path = os.path.join('Datasets/Accidentals', datestr)
